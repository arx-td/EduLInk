...
const ChooseUser = ({ visitor }) => {
  ...
  const navigateHandler = (user) => {
    if (user === "Admin") {
      if (visitor === "guest") {
        const email = "arxthedesigner2021@gmail.com";
        const fields = { email, password: "abdullah@1432" };
        setLoader(true);
      ...
      if (visitor === "guest") {
        ...
        const rollNum = "1";
        const fields = { rollNum, studentName, password: "987654321`" };
        setLoader(true);
        dispatch(loginUser(fields, user));
      } else {
      ...
      if (visitor === "guest") {
        const email = "amber@gmail.com";
        const fields = { email, password: "amber11" };
        setLoader(true);
        dispatch(loginUser(fields, user));
      } else {
...
const StyledContainer = styled.div`
  background: linear-gradient(to bottom, #201e43 #f06d06);
  height: 120vh;
  display: flex;
